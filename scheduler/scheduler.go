...
func (r *RoundRobin) Score(t task.Task, nodes []*node.Node) map[string]float64 {
	...
	for idx, node := range nodes {
		if idx == newWorker {
			nodeScores[node.Name] = 0.1 // Give the new worker the lowest score
		} else {
			nodeScores[node.Name] = 1.0 // Give all other workers a higher score
		}
	}
...
func (e *Epvm) SelectCandidateNodes(t task.Task, nodes []*node.Node) []*node.Node {
	...
	for node := range nodes {
		if checkDist(t, nodes[node].Disk-nodes[node].DiskAllocated) {
...
func checkDist(t task.Task, diskAvailable int64) bool {
